...
*/
		...
		else if(m_gamestate == UNLOADING)
		{
			...
			if(m_highlighted_area.contains(pos))
			{
				...
				if(t->can_unload())
				{
					this->process_action(units::UNLOAD, m_selection);
				}
				else
				{
					m_gamestate = IDLE;
...
//NOTE: Ersetzen
	...
	if(can_attack)
		menu->add_action(units::ATTACK);
	if(can_launch)
		menu->add_action(units::LAUNCH);
	if(can_capture)
		menu->add_action(units::CAPTURE);
	if(can_unload)
		menu->add_action(units::UNLOAD);
	if(can_explode)
		menu->add_action(units::EXPLODE);

	if(can_repair)
		menu->add_action(units::REPAIR);
	else if(can_supply)
		menu->add_action(units::SUPPLY);


	if(unit->can_hide())
	{
		if(unit->is_hidden())
			menu->add_action(units::APPEAR);
		else
			menu->add_action(units::HIDE);
	}
