	  ...
	  if(type == terrain::WATER)
		{
		  if(y > 0 && x > 0 && terrain::is_ground(scene->get_terrain(x-1, y-1)->type()))
			ret = get_path(terrain::SEASIDE, terrain::UL, terrain::CORNER); //Ecke oben Links
		  if(y < 19 && x > 0 && terrain::is_ground(scene->get_terrain(x-1, y+1)->type()))
			ret = get_path(terrain::SEASIDE, terrain::LD, terrain::CORNER); //Ecke unten links
		  if(y > 0 && x < 29 && terrain::is_ground(scene->get_terrain(x+1, y-1)->type()))
			ret = get_path(terrain::SEASIDE, terrain::UR, terrain::CORNER); //Ecke oben rechts
		  if(y < 19 && x < 29 && terrain::is_ground(scene->get_terrain(x+1, y+1)->type()))
			ret = get_path(terrain::SEASIDE, terrain::DR, terrain::CORNER); //Ecke unten rechts

		  if(x > 0 && terrain::is_ground(scene->get_terrain(x-1, y)->type()))
			ret = get_path(terrain::SEASIDE, terrain::L); //Küste links
		  if(x < 29 && terrain::is_ground(scene->get_terrain(x+1, y)->type()))
			ret = get_path(terrain::SEASIDE, terrain::R); //Küste rechts
		  if(y > 0 && terrain::is_ground(scene->get_terrain(x, y-1)->type()))
			ret = get_path(terrain::SEASIDE, terrain::U); //Küste oben
		  ...
		  if(x > 0 && y < 19 && y > 0 && scene->get_terrain(x-1, y)->type() == terrain::RIVER &&
			...
			ret = get_path(terrain::RIVER, terrain::L, terrain::MOUTH); //Flussmündung links

		  if(x < 29 && y < 19 && y > 0 && scene->get_terrain(x+1, y)->type() == terrain::RIVER &&
			 (scene->get_terrain(x+1, y-1)->type() != terrain::RIVER && scene->get_terrain(x+1, y+1)->type() != terrain::RIVER))
			ret = get_path(terrain::RIVER, terrain::R, terrain::MOUTH); //Flussmündung rechts

		  if(y > 0 && x < 29 && x > 0 && scene->get_terrain(x, y-1) ->type()== terrain::RIVER &&
			 (scene->get_terrain(x-1, y-1)->type() != terrain::RIVER && scene->get_terrain(x+1, y-1)->type() != terrain::RIVER))
			ret = get_path(terrain::RIVER, terrain::U, terrain::MOUTH); //Flussmündung oben

		  if(y < 19 && x < 29 && x > 0 && scene->get_terrain(x, y+1)->type() == terrain::RIVER &&
			 (scene->get_terrain(x-1, y+1)->type() != terrain::RIVER && scene->get_terrain(x+1, y+1)->type() != terrain::RIVER))
			ret = get_path(terrain::RIVER, terrain::D, terrain::MOUTH); //Flussmündung unten

		  if(x > 0 && terrain::is_ground(scene->get_terrain(x-1, y)->type()))
			{
			  if(y > 0 && terrain::is_ground(scene->get_terrain(x, y-1)->type()))
				ret = get_path(terrain::SEASIDE, terrain::UL); //Nach innen gebogene Küste (terrain::UL)

			  if(y < 19 && terrain::is_ground(scene->get_terrain(x, y+1)->type()))
				ret = get_path(terrain::SEASIDE, terrain::LD); //Nach innen gebogene Küste (LD)
			}

		  if(x < 29 && terrain::is_ground(scene->get_terrain(x+1, y)->type()))
			{
			  if(y > 0 && terrain::is_ground(scene->get_terrain(x, y-1)->type()))
				ret = get_path(terrain::SEASIDE, terrain::UR); //Nach innen gebogene Küste (UR)
			  else if(y < 19 && terrain::is_ground(scene->get_terrain(x, y+1)->type()))
				ret = get_path(terrain::SEASIDE, terrain::DR); //Nach innen gebogene Küste (DR)
			}

		  if(x > 0 && x < 29 && terrain::is_ground(scene->get_terrain(x-1, y)->type()) && terrain::is_ground(scene->get_terrain(x+1, y)->type()))
			{
			  if(y > 0 && terrain::is_ground(scene->get_terrain(x, y-1)->type()))
				ret = get_path(terrain::SEASIDE, terrain::ULR); //Bucht nach unten offen
			  else if(y < 19 && terrain::is_ground(scene->get_terrain(x, y+1)->type()))
				ret = get_path(terrain::SEASIDE, terrain::LDR); //Bucht nach oben offen
			}

		  if(y > 0 && y < 19 && terrain::is_ground(scene->get_terrain(x, y-1)->type()) && terrain::is_ground(scene->get_terrain(x, y+1)->type()))
			{
			  if(x > 0 && terrain::is_ground(scene->get_terrain(x-1, y)->type()))
				ret = get_path(terrain::SEASIDE, terrain::ULD); //Bucht nach rechts offen
			  else if(x < 29 && terrain::is_ground(scene->get_terrain(x+1, y)->type()))
				ret = get_path(terrain::SEASIDE, terrain::UDR); //Bucht nach links offen
			}
		  ...
		  if(y > 0 && y < 19 && x > 0 && x < 29)
			{
			  if(terrain::is_ground(scene->get_terrain(x, y-1)->type()) && //Oben
				...
				ret = get_path(terrain::SEASIDE, terrain::ULDR); //See (oder was das sein soll)
			}
		}
	  else if(type == terrain::RIVER)
		{
		  if(y > 0 && y < 19 && x > 0 && x < 29 &&
			...
			{
			  ret = get_path(terrain::RIVER, terrain::ULDR); //Flusskreuzung (ULDR)
			}
				  ...
				  is_connectable(terrain::RIVER, scene->get_terrain(x-1, y)->type())) //Links
			{
			  ret = get_path(terrain::RIVER, terrain::ULD); //Fluss mündet in anderen Fluss (terrain::ULD)
			}
				  ...
				  is_connectable(terrain::RIVER, scene->get_terrain(x+1, y)->type())) //Rechts
			{
			  ret = get_path(terrain::RIVER, terrain::UDR); //Fluss mündet in anderen Fluss (UDR)
			}
				  ...
				  is_connectable(terrain::RIVER, scene->get_terrain(x+1, y)->type())) //Rechts
			{
			  ret = get_path(terrain::RIVER, terrain::LDR); //Fluss mündet in anderen Fluss (LDR)
			}
				  ...
				  is_connectable(terrain::RIVER, scene->get_terrain(x+1, y)->type())) //Rechts
			{
			  ret = get_path(terrain::RIVER, terrain::ULR); //Fluss mündet in anderen Fluss (ULR)
			}
		  else if(y > 0 && x > 0 &&
				  is_connectable(terrain::RIVER, scene->get_terrain(x, y-1)->type()) && //Oben
				  is_connectable(terrain::RIVER, scene->get_terrain(x-1, y)->type())) //Links
			{
			  ret = get_path(terrain::RIVER, terrain::UL); //Kurve (terrain::UL)
			}
		  else if(y < 19 && x > 0 &&
				  is_connectable(terrain::RIVER, scene->get_terrain(x, y+1)->type()) && //Unten
				  is_connectable(terrain::RIVER, scene->get_terrain(x-1, y)->type())) //Links
			{
			  ret = get_path(terrain::RIVER, terrain::LD); //Kurve (LD)
			}
		  else if(y > 0 && x < 29 &&
				  is_connectable(terrain::RIVER, scene->get_terrain(x, y-1)->type()) && //Oben
				  is_connectable(terrain::RIVER, scene->get_terrain(x+1, y)->type())) //Rechts
			{
			  ret = get_path(terrain::RIVER, terrain::UR); //Kurve (UR)
			}
		  else if(y < 19 && x < 29 &&
				  is_connectable(terrain::RIVER, scene->get_terrain(x, y+1)->type()) && //Unten
				  is_connectable(terrain::RIVER, scene->get_terrain(x+1, y)->type())) //Rechts
			{
			  ret = get_path(terrain::RIVER, terrain::DR); //Kurve (DR)
			}
			...
			{
			  ret = get_path(terrain::RIVER, terrain::UD); //Gerade (UD)
			}
			...
			{
			  ret = get_path(terrain::RIVER, terrain::LR); //Gerade (UD)
			}
	  ...
	  else if(type == terrain::STREET)
		{
		  if(y > 0 && y < 19 && x > 0 && x < 29 &&
			...
			{
			  ret = get_path(terrain::STREET, terrain::ULDR); //Kreuzung (ULDR)
			}
		  else if(y > 0 && y < 19 && x > 0 &&
				  is_connectable(terrain::STREET, scene->get_terrain(x, y-1)->type()) && //Oben
				  is_connectable(terrain::STREET, scene->get_terrain(x, y+1)->type()) && //Unten
				  is_connectable(terrain::STREET, scene->get_terrain(x-1, y)->type())) //Links
			{
			  ret = get_path(terrain::STREET, terrain::ULD); //Strasse trifft auf querstrasse (terrain::ULD)
			}
		  else if(y > 0 && y < 19 && x < 29 &&
				  is_connectable(terrain::STREET, scene->get_terrain(x, y-1)->type()) && //Oben
				  is_connectable(terrain::STREET, scene->get_terrain(x, y+1)->type()) && //Unten
				  is_connectable(terrain::STREET, scene->get_terrain(x+1, y)->type())) //Rechts
			{
			  ret = get_path(terrain::STREET, terrain::UDR); //Strasse trifft auf querstrasse (UDR)
			}
		  else if(y < 19 && x > 0 && x < 29 &&
				  is_connectable(terrain::STREET, scene->get_terrain(x, y+1)->type()) && //Unten
				  is_connectable(terrain::STREET, scene->get_terrain(x-1, y)->type()) &&//Links
				  is_connectable(terrain::STREET, scene->get_terrain(x+1, y)->type())) //Rechts
			{
			  ret = get_path(terrain::STREET, terrain::LDR); //Strasse trifft auf querstrasse (LDR)
			}
		  else if(y > 0 && x > 0 && x < 29 &&
				  is_connectable(terrain::STREET, scene->get_terrain(x, y-1)->type()) && //Oben
				  is_connectable(terrain::STREET, scene->get_terrain(x-1, y)->type()) &&//Links
				  is_connectable(terrain::STREET, scene->get_terrain(x+1, y)->type())) //Rechts
			{
			  ret = get_path(terrain::STREET, terrain::ULR); //Strasse trifft auf querstrasse (ULR)
			}
		  else if(y > 0 && x > 0 &&
				  is_connectable(terrain::STREET, scene->get_terrain(x, y-1)->type()) && //Oben
				  is_connectable(terrain::STREET, scene->get_terrain(x-1, y)->type())) //Links
			{
			  ret = get_path(terrain::STREET, terrain::UL); //Kurve (terrain::UL)
			}
		  else if(y < 19 && x > 0 &&
				  is_connectable(terrain::STREET, scene->get_terrain(x, y+1)->type()) && //Unten
				  is_connectable(terrain::STREET, scene->get_terrain(x-1, y)->type())) //Links
			{
			  ret = get_path(terrain::STREET, terrain::LD); //Kurve (LD)
			}
		  else if(y > 0 && x < 29 &&
				  is_connectable(terrain::STREET, scene->get_terrain(x, y-1)->type()) && //Oben
				  is_connectable(terrain::STREET, scene->get_terrain(x+1, y)->type())) //Rechts
			{
			  ret = get_path(terrain::STREET, terrain::UR); //Kurve (UR)
			}
		  else if(y < 19 && x < 29 &&
				  is_connectable(terrain::STREET, scene->get_terrain(x, y+1)->type()) && //Unten
				  is_connectable(terrain::STREET, scene->get_terrain(x+1, y)->type())) //Rechts
			{
			  ret = get_path(terrain::STREET, terrain::DR); //Kurve (DR)
			}
			...
			{
			  ret = get_path(terrain::STREET, terrain::UD); //Gerade (UD)
			}
			...
			{
			  ret = get_path(terrain::STREET, terrain::LR); //Gerade (UD)
			}
		  else
			{
			  ret = get_path(terrain::STREET, terrain::LR); //Gerade (LR)
			}
		}
	  else if(type == terrain::BRIDGE)
		{
		  if((y > 0 && is_connectable(terrain::BRIDGE, scene->get_terrain(x, y-1)->type())) || //Oben
			...
			{
			  ret = get_path(terrain::BRIDGE, terrain::UD); //Gerade (UD)
			}
			...
			{
			  ret = get_path(terrain::BRIDGE, terrain::LR); //Gerade (LR)
			}
		}
	  else if(type == terrain::PIPE)
		{
		  if(y > 0 && x > 0 &&
			...
			{
			  ret = get_path(terrain::PIPE, terrain::UL); //Kurve (terrain::UL)
			}
		  else if(y < 19 && x > 0 &&
				  is_connectable(terrain::PIPE, scene->get_terrain(x, y+1)->type()) && //Unten
				  is_connectable(terrain::PIPE, scene->get_terrain(x-1, y)->type())) //Links
			{
			  ret = get_path(terrain::PIPE, terrain::LD); //Kurve (LD)
			}
		  else if(y > 0 && x < 29 &&
				  is_connectable(terrain::PIPE, scene->get_terrain(x, y-1)->type()) && //Oben
				  is_connectable(terrain::PIPE, scene->get_terrain(x+1, y)->type())) //Rechts
			{
			  ret = get_path(terrain::PIPE, terrain::UR); //Kurve (UR)
			}
		  else if(y < 19 && x < 29 &&
				  is_connectable(terrain::PIPE, scene->get_terrain(x, y+1)->type()) && //Unten
				  is_connectable(terrain::PIPE, scene->get_terrain(x+1, y)->type())) //Rechts
			{
			  ret = get_path(terrain::PIPE, terrain::DR); //Kurve (DR)
			}
			...
			{
			  ret = get_path(terrain::PIPE, terrain::LR); //Gerade (LR)
			}
			...
			{
			  ret = get_path(terrain::PIPE, terrain::UD); //Gerade (UD)
			}
		  else if(y > 0 && is_connectable(terrain::PIPE, scene->get_terrain(x, y-1)->type())) //Oben
			{
			  ret = get_path(terrain::PIPE, terrain::U); //Endstück (terrain::U)
			}
		  else if(y < 19 && is_connectable(terrain::PIPE, scene->get_terrain(x, y+1)->type())) //Unten
			{
			  ret = get_path(terrain::PIPE, terrain::D); //Endstück (terrain::D)
			}
		  else if(x > 0 && is_connectable(terrain::PIPE, scene->get_terrain(x-1, y)->type())) //Links
			{
			  ret = get_path(terrain::PIPE, terrain::L); //Endstück (terrain::L)
			}
		  else if(x < 29 && is_connectable(terrain::PIPE, scene->get_terrain(x+1, y)->type())) //Links
			{
			  ret = get_path(terrain::PIPE, terrain::R); //Endstück (terrain::R)
			}
		  else
			{
			  ret = get_path(terrain::PIPE, terrain::LR); //Wenn keine Verbindungen möglich sind (LR)
			}
		}
	  else if(type == terrain::SEGMENT_PIPE)
		{
		  if(x > 0 && is_connectable(terrain::SEGMENT_PIPE, scene->get_terrain(x-1, y)->type()) && //Links
			...
			{
			  ret = get_path(terrain::SEGMENT_PIPE, terrain::LR); //Gerade (LR)
			}
			...
			{
			  ret = get_path(terrain::SEGMENT_PIPE, terrain::UD); //Gerade (UD)
			}
		}
	  else if(type == terrain::WRECKAGE)
		{
		  if((y > 0 && is_connectable(type, scene->get_terrain(x, y-1)->type())) ||
			...
			{
			  ret = get_path(terrain::WRECKAGE, terrain::UD);
			}
		  else if((x > 0 && is_connectable(type, scene->get_terrain(x-1, y)->type())) ||
				  (x < 29 && is_connectable(type, scene->get_terrain(x+1, y)->type())))
			{
			  ret = get_path(terrain::WRECKAGE, terrain::LR);
			}
		  else
			{
			  //Trümmer
			  ret = get_path(terrain::WRECKAGE, terrain::UNDEF);
			}
		}
	  else if(type == terrain::MOUNTAIN)
		{
		  if(y > 0)
			{
			  ret = get_path(terrain::MOUNTAIN, terrain::UNDEF, terrain::BIG);
			}
		  else
			{
			  ret = get_path(terrain::MOUNTAIN, terrain::UNDEF, terrain::SMALL);
			}
		}
	  else if(type == terrain::BEACH)
		{
		  ...
		  if(x > 0 && terrain::is_ground(scene->get_terrain(x-1, y)->type()))
			ret = get_path(terrain::BEACH, terrain::L); //Küste links
		  if(x < 29 && terrain::is_ground(scene->get_terrain(x+1, y)->type()))
			ret = get_path(terrain::BEACH, terrain::R); //Küste rechts
		  if(y > 0 && terrain::is_ground(scene->get_terrain(x, y-1)->type()))
			ret = get_path(terrain::BEACH, terrain::U); //Küste oben
		  if(y < 19 && terrain::is_ground(scene->get_terrain(x, y+1)->type()))
			ret = get_path(terrain::BEACH, terrain::D); //Küste unten


		  if(x > 0 && terrain::is_ground(scene->get_terrain(x-1, y)->type()))
			{
			  if(y > 0 && terrain::is_ground(scene->get_terrain(x, y-1)->type()))
				ret = get_path(terrain::BEACH, terrain::UL); //Nach innen gebogene Küste (terrain::UL)

			  if(y < 19 && terrain::is_ground(scene->get_terrain(x, y+1)->type()))
				ret = get_path(terrain::BEACH, terrain::LD); //Nach innen gebogene Küste (LD)
			}

		  if(x < 29 && terrain::is_ground(scene->get_terrain(x+1, y)->type()))
			{
			  if(y > 0 && terrain::is_ground(scene->get_terrain(x, y-1)->type()))
				ret = get_path(terrain::BEACH, terrain::UR); //Nach innen gebogene Küste (UR)

			  if(y < 19 && terrain::is_ground(scene->get_terrain(x, y+1)->type()))
				ret = get_path(terrain::BEACH, terrain::DR); //Nach innen gebogene Küste (DR)
			}
		  ...
		  if(y > 0 && y < 19 &&
			...
			{
			  if(x > 0 && terrain::is_ground(scene->get_terrain(x-1, y)->type()))
				ret = get_path(terrain::BEACH, terrain::ULD); //Bucht mit Strand
			  else if(x < 29 && terrain::is_ground(scene->get_terrain(x+1, y)->type()))
				ret = get_path(terrain::BEACH, terrain::UDR); //Bucht mit Strand
			}
		  ...
		  if(x > 0 && x < 29 &&
			...
			{
			  if(y > 0 && terrain::is_ground(scene->get_terrain(x, y-1)->type()))
				ret = get_path(terrain::BEACH, terrain::ULR); //Bucht mit Strand
			  else if(y < 19 && terrain::is_ground(scene->get_terrain(x, y+1)->type()))
				ret = get_path(terrain::BEACH, terrain::LDR); //Bucht mit Strand
			}
		}
	  else if(type != terrain::WATER)
		{
		  ret = get_path(scene->get_terrain(x, y)->type(), terrain::UNDEF, extra);
		}
