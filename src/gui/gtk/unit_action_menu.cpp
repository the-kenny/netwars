...
: Gtk::Window(Gtk::WINDOW_POPUP), m_return(false), m_return_value(CANCEL)
{
	m_vbox = Gtk::manage(new Gtk::VBox);
	m_vbox->set_spacing(1);

	Gtk::Button *cancel = Gtk::manage(new Gtk::Button("Cancel"));
	cancel->signal_clicked().connect(sigc::bind(sigc::mem_fun(*this, &unit_action_menu::on_button_click), CANCEL));
...
unit_action_menu::actions unit_action_menu::run()
{
	int x = 0;
	int y = 0;
...
void unit_action_menu::add_action(actions a)
{
	std::string label("");
